...
class Refund(models.Model):
    """One independent refund lifecycle belonging to a Payment.

    Architectural boundary
    ----------------------
    Refund is a domain entity. It represents one refund lifecycle and owns
    only refund-local financial state and invariants.

    It deliberately does NOT:

    * call a gateway or perform HTTP;
    * query repositories or aggregate other refunds;
    * open transactions or acquire locks;
    * mutate Payment or Order;
    * dispatch events/tasks;
    * persist itself from domain commands.

    The application service owns orchestration, transaction boundaries,
    Payment locking, cumulative refund checks, idempotency coordination and
    gateway interaction.

    Concurrency rule
    ---------------
    The refundable balance is an aggregate invariant and MUST be calculated
    by the application workflow while holding the canonical Payment row lock.
    This model intentionally does not try to calculate that balance itself.
    """
    ...
    # Immutable financial snapshot for this Refund lifecycle.
    ...
    # Request-level idempotency. The application layer must reuse this key
    # when the same logical refund request is retried.
    ...
    # Gateway identities are deliberately scoped by Payment at the database
    # level below. Their global uniqueness is provider-specific and therefore
    ...
    def is_full_payment_refund(self, *, payment_amount: Decimal) -> bool:
        """Return whether this Refund equals the original Payment amount.

        This only describes this Refund. It does not prove that the Payment
        has been fully refunded because other successful Refunds may exist.
        """
    ...
    def validate_against_payment(
        ...
        *,
        ...
        This method intentionally does not dereference ``self.payment`` and
        therefore cannot introduce an implicit query. Cumulative refund
        validation belongs to the service while the Payment is locked.
        """
        errors: dict[str, object] = {}
        normalized_amount = Decimal(str(payment_amount))
        normalized_currency = self._normalize(payment_currency)
        ...
        if self.currency != normalized_currency:
            errors["currency"] = _(
    ...
    def mark_success(
        ...
        *,
        ...
        """Move PENDING -> SUCCESS or idempotently reconcile SUCCESS.

        A repeated SUCCESS is accepted only when all supplied gateway
        identities are compatible with the already-recorded identities.
        Terminal financial state is never rewritten.
        """
        ...
        if self.is_success:
            self._reconcile_success_identity(
                ...
                gateway_transaction_id=transaction,
            )
            return self
    ...
    def mark_failed(
        ...
        *,
        ...
        FAILED is terminal. Repeating the same domain command is a strict
        no-op; it does not rewrite historical failure evidence.
        """
        if self.is_failed:
            return self
    ...
    def register_gateway_response(
        ...
        *,
        ...
        """Record normalized non-terminal gateway metadata.

        This is deliberately restricted to PENDING because terminal refund
        records are historical financial facts and must not be rewritten.
        """
        self._require(
            self.is_pending,
    ...
    def _reconcile_success_identity(
        ...
        *,
        ...
        """Validate a repeated SUCCESS without mutating terminal state."""
        if gateway_reference and self.gateway_reference:
            self._require(
                self.gateway_reference == gateway_reference,
                _("Gateway refund reference conflict detected."),
            )
        elif gateway_reference and not self.gateway_reference:
            # A SUCCESS record should normally already have an identity due
            # to the database constraint. This branch exists for defensive
            # in-memory use and deliberately does not mutate terminal state.
            self._require(
                False,
                _("Successful refund is missing its recorded gateway reference."),
            )

        if gateway_transaction_id and self.gateway_transaction_id:
            self._require(
                self.gateway_transaction_id == gateway_transaction_id,
                _("Gateway refund transaction identifier conflict detected."),
            )
        elif gateway_transaction_id and not self.gateway_transaction_id:
            self._require(
                False,
                _(
                    "Successful refund is missing its recorded gateway "
                    "transaction identifier."
                ),
            )

        # If the callback supplies neither identity, there is no new identity
        # evidence to compare. The already-persisted SUCCESS remains intact.
